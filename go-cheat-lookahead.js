/**
 * IPvGO opponent-aware shallow lookahead player with conservative two-move cheats.
 *
 * Opens with playTwoMoves() when its success chance is 100%. Afterward, it repeats
 * only while the current chance is at least 95%, then uses normal lookahead moves.
 *
 * Usage:
 *   run go-cheat-lookahead.js --opponent "Daedalus" --size 7 --games 10
 *   run go-cheat-lookahead.js --opponent "Illuminati" --size 7 --games 0 --verbose --candidates 12 --replies 6
 *
 * Named flags:
...
export async function main(ns) {
  ...
  ns.clearLog();
  ns.ui.openTail();
  ns.ui.setTailTitle(`IPvGO cheat lookahead ${opponent} ${size}x${size}`);
  ...
  while (games === 0 || played < games) {
    ...
    while (ns.go.getCurrentPlayer() !== "None") {
      ...
      const mayCheat = shouldUseCheat(cheatChance, cheatAttempts);
      const cheatPair = mayCheat ? chooseCheatPair(board, valid, options) : null;

      if (cheatPair) {
        const result = await ns.go.cheat.playTwoMoves(cheatPair.first.x, cheatPair.first.y, cheatPair.second.x, cheatPair.second.y);
        cheatAttempts++;
        turns++;
        passes = 0;
        ns.print(
          `Cheat ${cheatAttempts}: ${coord(cheatPair.first.x, cheatPair.first.y)} + ${coord(cheatPair.second.x, cheatPair.second.y)} chance=${(cheatChance * 100).toFixed(2)}% score=${cheatPair.score.toFixed(2)} -> ${result.type}`,
        );
      } else {
...
export function shouldUseCheat(chance, priorAttempts) {
  return priorAttempts === 0 ? chance >= 1 : chance >= 0.95;
}

function chooseCheatPair(board, valid, options) {
  // Unlike normal Go moves, playTwoMoves places both stones simultaneously and permits
  // otherwise-illegal placements. Rank every empty online point, then simulate pairs with
  // the cheat's actual capture order.
  const points = [];
  for (let x = 0; x < board.length; x++) {
    for (let y = 0; y < board[x].length; y++) {
      if (board[x][y] !== ".") continue;
      const single = simulateMove(board, x, y, "X");
      const rank = single ? scoreMove(board, single, x, y, "X") : moveShapeBonus(board, x, y, "X");
      points.push({ x, y, rank });
    }
  }
  points.sort((a, b) => b.rank - a.rank);
  const candidates = points.slice(0, Math.max(2, options.candidates));
  let best = null;
  ...
  for (let i = 0; i < candidates.length; i++) {
    for (let j = i + 1; j < candidates.length; j++) {
      ...
      const sim = simulateTwoMoves(board, first, second, "X");
      const reply = chooseLikelyReply(sim.board, options);
      const afterReply = reply ? reply.blackScoreAfterReply : evaluateBoard(sim.board);
      const score = afterReply + sim.captured * 30 + (first.rank + second.rank) * 0.08;
      const candidate = { first, second, score };
...
export function simulateTwoMoves(board, first, second, color) {
  const b = board.map((col) => col.split(""));
  b[first.x][first.y] = color;
  b[second.x][second.y] = color;
  const opponent = color === "X" ? "O" : "X";
  let captured = removeDeadGroups(b, opponent);
  // The game removes friendly dead groups only when no enemy group was captured.
  if (!captured) removeDeadGroups(b, color);
  return { board: b.map((col) => col.join("")), captured };
}

function removeDeadGroups(boardArray, color) {
  const seen = new Set();
  const dead = [];
  for (let x = 0; x < boardArray.length; x++) {
    for (let y = 0; y < boardArray[x].length; y++) {
      if (boardArray[x][y] !== color || seen.has(`${x},${y}`)) continue;
      const group = collectGroup(boardArray, x, y);
      for (const point of group) seen.add(`${point.x},${point.y}`);
      if (countLiberties(boardArray, group) === 0) dead.push(...group);
    }
  }
  for (const point of dead) boardArray[point.x][point.y] = ".";
  return dead.length;
}
...
function printUsage(ns) {
  ns.tprintf('Usage: run go-cheat-lookahead.js --opponent "Netburners" --size 5 --games 10');
  ns.tprintf('       run go-cheat-lookahead.js --opponent "Daedalus" --size 7 --games 0 --verbose --candidates 12 --replies 6');
  ns.tprintf("Flags:");
